...
class DatabaseHelper {
  ...
  static const String tableBreedingEvents = 'breeding_events';
  static const String tableKiddingRecords = 'kidding_records';
  ...
  static const String tableMilkingRecords = 'milking_records';
    ...
    await db.execute('''
      CREATE TABLE $tableBreedingEvents (
        ...
        method              TEXT DEFAULT 'natural'
                            CHECK(method IN ('natural','ai','embryo_transfer')),
        confirmed_pregnant  INTEGER NOT NULL DEFAULT 0,
        confirmation_date   TEXT,
        confirmation_method TEXT,
        outcome             TEXT CHECK(outcome IN ('kidded','open','aborted','unknown')),
        notes               TEXT,
        created_at          TEXT NOT NULL DEFAULT (datetime('now')),
        updated_at          TEXT NOT NULL DEFAULT (datetime('now')),
        FOREIGN KEY (buck_id) REFERENCES $tableAnimals(id) ON DELETE SET NULL,
        FOREIGN KEY (doe_id)  REFERENCES $tableAnimals(id) ON DELETE CASCADE
      )
    ''');

    await db.execute(
      'CREATE INDEX idx_breeding_doe ON $tableBreedingEvents(doe_id)',
    );
    await db.execute(
      'CREATE INDEX idx_breeding_buck ON $tableBreedingEvents(buck_id)',
    );
    await db.execute(
      'CREATE INDEX idx_breeding_date ON $tableBreedingEvents(breeding_date)',
    );
  }
  ...
  Future<void> _createKiddingRecordsTable(dynamic db) async {
    await db.execute('''
      CREATE TABLE $tableKiddingRecords (
        ...
        litter_size         INTEGER,
        birth_weight_lbs    REAL,
        sex                 TEXT CHECK(sex IN ('doe','buck','unknown')),
        birth_type          TEXT CHECK(birth_type IN ('single','twin','triplet','quad','other')),
        presentation        TEXT CHECK(presentation IN ('normal','malpresentation','assisted')),
        survival_status     TEXT DEFAULT 'alive'
                            CHECK(survival_status IN ('alive','died_at_birth','died_within_24h',
                                                       'died_within_week','died_later','sold')),
        received_colostrum  INTEGER DEFAULT 0,
        bottle_fed          INTEGER DEFAULT 0,
        dam_condition_score INTEGER CHECK(dam_condition_score BETWEEN 1 AND 5),
        complications       TEXT,
        notes               TEXT,
        created_at          TEXT NOT NULL DEFAULT (datetime('now')),
        updated_at          TEXT NOT NULL DEFAULT (datetime('now')),
        FOREIGN KEY (breeding_event_id) REFERENCES $tableBreedingEvents(id) ON DELETE SET NULL,
        FOREIGN KEY (doe_id)  REFERENCES $tableAnimals(id) ON DELETE CASCADE,
        FOREIGN KEY (buck_id) REFERENCES $tableAnimals(id) ON DELETE SET NULL,
        FOREIGN KEY (kid_id)  REFERENCES $tableAnimals(id) ON DELETE SET NULL
      )
    ''');

    await db.execute(
      'CREATE INDEX idx_kidding_doe ON $tableKiddingRecords(doe_id)',
    );
    await db.execute(
      'CREATE INDEX idx_kidding_date ON $tableKiddingRecords(kidding_date)',
    );
  }
